...
function playRound(playerSelection){
    ...
    if (computerSelection === playerSelection){
        ...
        score.textContent = (`Score: Player ${playerScore} - ${computerScore} Computer`);
        if (playerScore === 5) {
            result.innerHTML = 'You got to five first.You won the game! &#129327. Hit the Restart button to play again'
            stopPlay();
        ...
        score.textContent = (`Score: Player ${playerScore} - ${computerScore} Computer`);
        if (computerScore === 5) {
            result.innerHTML = 'The Computer got to five first.You lose! &#128541. Hit the Restart button to try again'
            stopPlay();
            newGameBtn.style.display = 'inline-block';
        } 
    }
